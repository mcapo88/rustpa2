...
impl <'a>Shell<'a> {
    ...
    fn run_cmd(&self, cm_line: &str) {
        let cmd_line = cm_line.to_string().clone();
        thread::spawn(move || {
        //create input and output threads inside here somewhere
        let arg: Vec<&str> = cmd_line.split(' ').filter_map(|x| {
            if x == "" {
                None
            } else {
                Some(x)
        ...
        if Shell::cmd_exists(program) {
            ...
            let output = Command::new(program).args(argv).output().unwrap_or_else(|e| {panic!("failed to execute process: {}", e)});//creates its own process
